"""中国行政区划数据：省/直辖市 + 地级市
source: metowolf/iplist docs/cncity.md
生成于解析时刻；34 省级单位，336 个城市/地区。
每个 code 对应可从 https://metowolf.github.io/iplist/data/cncity/{code}.txt 拉取的IP段。
"""
...
PROVINCES: list[dict] = [
    ...
    {"code": "650000", "name": "新疆维吾尔自治区", "cities": [
        ...
        {"code": "659000", "name": "省直辖县(*)"},
    ]},
    {"code": "710000", "name": "台湾省", "cities": [
    ]},
    {"code": "810000", "name": "香港特别行政区", "cities": [
    ]},
    {"code": "820000", "name": "澳门特别行政区", "cities": [
    ]},
]


def build_indexes() -> tuple[dict[str, str], dict[str, str]]:
    """构建：code→name 的扁平表，code→province_name 表（市级映射到省名）。"""
    code_to_name: dict[str, str] = {}
    code_to_province: dict[str, str] = {}
    for p in PROVINCES:
        code_to_name[p["code"]] = p["name"]
        code_to_province[p["code"]] = p["name"]
        for c in p["cities"]:
            code_to_name[c["code"]] = c["name"]
            code_to_province[c["code"]] = p["name"]
    return code_to_name, code_to_province


CODE_TO_NAME, CODE_TO_PROVINCE = build_indexes()


# 行政区划后缀，按"长优先"匹配（避免"内蒙古自治区"被吃成"内"）
_SUFFIXES = (
    "维吾尔自治区", "壮族自治区", "回族自治区",
    "特别行政区", "自治区", "自治州", "自治县",
    "省", "市", "地区", "盟", "旗",
...
def group_label_for(code: str) -> str:
    """构造爱快上的"省_市"风格分组名（不含 GEO_ 前缀）。

    - 市级（210200 大连市，所在省 辽宁省）→ "辽宁_大连"
    - 省级（210000 辽宁省）→ "辽宁"
    - 直辖市（110000 北京市，下面无市）→ "北京"
    - 未知 code → 原样返回
    """
    name = CODE_TO_NAME.get(code)
    if not name:
        return code
    if is_province_code(code):
        return short_name(name)
    province_name = CODE_TO_PROVINCE.get(code, "")
    prov = short_name(province_name) if province_name else ""
    city = short_name(name)
    return f"{prov}_{city}" if prov else city
